...
function mrk_content_types() {
	...
	register_taxonomy( 'mrk_service_area', array( 'mrk_service' ), array( 'labels' => array( 'name' => __( 'Service areas', 'mrk-digital' ), 'singular_name' => __( 'Service area', 'mrk-digital' ) ), 'public' => true, 'hierarchical' => true, 'rewrite' => array( 'slug' => 'service-area' ), 'show_in_rest' => true ) );
	register_taxonomy( 'mrk_project_type', array( 'mrk_project' ), array( 'labels' => array( 'name' => __( 'Project types', 'mrk-digital' ), 'singular_name' => __( 'Project type', 'mrk-digital' ) ), 'public' => true, 'hierarchical' => true, 'rewrite' => array( 'slug' => 'project-type' ), 'show_in_rest' => true ) );
}
...
function mrk_customize( $wp_customize ) {
	...
	foreach ( array( 'whatsapp' => 'WhatsApp number (international digits only)', 'contact_email' => 'Contact email', 'service_area' => 'Service area text', 'public_address' => 'Optional public business address' ) as $id => $label ) {
		$sanitize = 'contact_email' === $id ? 'sanitize_email' : ( 'public_address' === $id ? 'sanitize_textarea_field' : 'sanitize_text_field' );
		$wp_customize->add_setting( 'mrk_' . $id, array( 'sanitize_callback' => $sanitize ) );
		$wp_customize->add_control( 'mrk_' . $id, array( 'label' => __( $label, 'mrk-digital' ), 'section' => 'mrk_contact', 'type' => 'contact_email' === $id ? 'email' : ( 'public_address' === $id ? 'textarea' : 'text' ) ) );
	}
}
...
function mrk_meta() { if ( is_admin() ) return; $description = is_singular() ? get_post_meta( get_queried_object_id(), '_mrk_meta_description', true ) : ''; if ( ! $description ) { $description = is_singular() ? get_the_excerpt() : get_bloginfo( 'description' ); } if ( ! $description ) $description = 'Web development, digital services, PLC, Arduino and automation solutions from MRK Digital.'; echo '<meta name="description" content="' . esc_attr( wp_strip_all_tags( $description ) ) . '">' . "\n"; echo '<meta property="og:site_name" content="MRK Digital &amp; Online Services Center">' . "\n"; echo '<meta property="og:type" content="' . ( is_singular( 'post' ) ? 'article' : 'website' ) . '">' . "\n"; echo '<meta name="twitter:card" content="summary_large_image">' . "\n"; }
...
function mrk_handle_quote() { if ( ! isset( $_POST['mrk_quote_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mrk_quote_nonce'] ) ), 'mrk_quote' ) ) wp_die( esc_html__( 'Security check failed.', 'mrk-digital' ) ); if ( ! empty( $_POST['website'] ) ) { wp_safe_redirect( add_query_arg( 'quote', 'sent', wp_get_referer() ?: home_url('/contact/') ) ); exit; } $name=sanitize_text_field(wp_unslash($_POST['name']??'')); $contact=sanitize_text_field(wp_unslash($_POST['contact']??'')); $email=sanitize_email(wp_unslash($_POST['email']??'')); $service=sanitize_text_field(wp_unslash($_POST['service']??'')); $budget=sanitize_text_field(wp_unslash($_POST['budget']??'')); $preferred_contact=sanitize_text_field(wp_unslash($_POST['preferred_contact']??'')); $description=sanitize_textarea_field(wp_unslash($_POST['description']??'')); if ( ! $name || ! $contact || ! $description || ( ! empty( $_POST['email'] ) && ! is_email( $email ) ) ) { wp_safe_redirect( add_query_arg( 'quote', 'invalid', wp_get_referer() ?: home_url('/contact/') ) ); exit; } $to=get_theme_mod('mrk_contact_email') ?: get_option('admin_email'); wp_mail($to, sprintf('[MRK Quote] %s — %s',$service?:'General enquiry',$name), "Name: $name\nContact: $contact\nEmail: $email\nService: $service\nBudget: $budget\nPreferred contact: $preferred_contact\n\nProject: $description"); wp_safe_redirect( add_query_arg( 'quote', 'sent', wp_get_referer() ?: home_url('/contact/') ) ); exit; }
